const bigTriangleSide = 300; // Big equilateral triangle side size
...
* Fill activity log list.
* */
function fillActivityLog(activityLogList) {
    const activityLog = document.getElementById("activityLog");
    activityLog.innerHTML = "";
    for (let i = activityLogList.length - 1; i >= 0; i--) {
        activityLog.innerHTML += "<li>" + activityLogList[i] + "</li>"
    }
}
...
function drawCanvas(boardState) {
    ...
    // Fill activity log
    fillActivityLog(boardState['log_activity']);
}
...
function saveCanvas() {
    ...
    const response_json = JSON.parse(xmlHttp.responseText);
    // Show error or show success
    if (response_json.hasOwnProperty('error')) {
        document.getElementById('modalErrorText').innerHTML = response_json['error'];
        $('#modalError').modal('toggle');
    } else if (response_json.hasOwnProperty('success')) {
...
* Clears board state and returns a white board state.
* */
function clearCanvas() {
    let xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", 'http://127.0.0.1:8000/api/clearCanvas?nTrianglesBottom=' + nTrianglesBottom, false);
    xmlHttp.send(null);

    // Show error or draw canvas
    const response_json = JSON.parse(xmlHttp.responseText);
    if (response_json.hasOwnProperty('error')) {
        document.getElementById('modalErrorText').innerHTML = response_json['error'];
        $('#modalError').modal('toggle');
...
* Clear a single digit in personal ID.
* */
function clearSingleInput() {
    let xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", 'http://127.0.0.1:8000/api/clearSingleInput', false);
    xmlHttp.send(null);
    const response_json = JSON.parse(xmlHttp.responseText);
...
/* FUNCTION: clearEntireInput
* Clear entire personal ID input.
* */
function clearEntireInput() {
    let xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", 'http://127.0.0.1:8000/api/clearEntireInput', false);
...
function clearColor() {
    let xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", 'http://127.0.0.1:8000/api/clearColor', false);
...
window.setInterval(function () {
    ...
    xmlHttp.onload = function () {
        ...
        if (this.status >= 200 && this.status < 300) {
            ...
            document.getElementById('personalId').value = response['personal_id'];

            if (response['color'] === 'red') {
                document.getElementById('redColor').checked = true;
